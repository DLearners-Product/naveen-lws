...
public class SGM_text : MonoBehaviour
{
    [Header("DB")]
    public string URL;
  ...
  //  public List<Sprite> SPRL_optionsSprite;
    ...
    public void BUT_optionClick()
    {
        ...
        if (STR_clickedAnswer.Contains(STR_currentCorrectAnswer))
        {
            ...
            IM_answerDisplay.sprite = SPRL_questionSprite[I_currentQuestionCount];
    ...
    public void THI_showQuestion()
    {
        ...
        if (I_currentQuestionCount < I_totalQuestionCount)
        {
            ...
            for (int i = z; i <= I_currentOptionReqCount; i++)
            {
                ...
                {
                    STR_3rdOptionName = STRL_optionsDB[i];
                }
                if (STR_1stOptionName != "" && STR_2ndOptionName == "")
                {
                    STR_2ndOptionName = STRL_optionsDB[i];
                }
                if (STR_1stOptionName == "")
                {
                    STR_1stOptionName = STRL_optionsDB[i];
                }
            }
            GA_options[0].GetComponent<Text>().text = STRL_optionsDB[I_1stSprite];
            GA_options[1].GetComponent<Text>().text = STRL_optionsDB[I_2ndSprite];
            GA_options[2].GetComponent<Text>().text = STRL_optionsDB[I_3rdSprite];
