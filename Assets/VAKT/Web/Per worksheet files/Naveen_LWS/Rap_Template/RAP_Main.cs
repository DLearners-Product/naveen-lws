...
using TMPro;
public class RAP_Main : MonoBehaviour
{
    ...
    public GameObject G_RAP_Template;
    public ToggleGroup TG_Toggle;
    bool B_Empty;
    ...
    public void THI_ShowQuestion()
    {
        ...
        {
            G_RAP_Template.SetActive(false);
            I_questionCount++;
            int I_Qcount = I_questionCount + 1;
            TEX_questionCount.text = "" + I_Qcount + "/" + (STRL_questions.Count);
            STR_currentQuestionID = STRL_questionID[I_questionCount];
            TMP_Passage.text = STRL_questions[I_questionCount];
    ...
    public void BUT_WorksheetCompleted()
    {
       ...
       // STRL_gameData = new List<string>();
        ...
        for (int i = 0; i < IFA_Objects.Length; i++)
        {
           // Debug.Log(IFA_Objects[i].name + " in for");
            if (IFA_Objects[i].text == "")
            {
               // Debug.Log(IFA_Objects[i].name + " in if");
                B_Empty = true;
                G_errorMSG.SetActive(true);
                return;
               // STR_selectedAnswer += IFA_Objects[i].name + " = " + IFA_Objects[i].text;
            }
        }

        if(!B_Empty)
        {
          //  Debug.Log(B_Empty + " in bool");
            G_errorMSG.SetActive(false);
           
            THI_NEWJson();
            Invoke(nameof(THI_ShowQuestion), 1f);
        }
    ...
    {
        public string Main_Idea;
        public string Detail_1;
        public string Detail_2;
        public string Detail_3;
        public string Detail_4;
        public string Put_in_our_own_words;
    }
    public void THI_NEWJson()
    {
        NEWJson typing = new NEWJson();
        typing.Main_Idea = IFA_Objects[0].text;
        typing.Detail_1 = IFA_Objects[1].text;
        typing.Detail_2 = IFA_Objects[2].text;
        typing.Detail_3 = IFA_Objects[3].text;
        typing.Detail_4 = IFA_Objects[4].text;
        typing.Put_in_our_own_words = IFA_Objects[5].text;

        string toJson = JsonUtility.ToJson(typing);
        List<string> Answer = new List<string>();
        Answer.Add(toJson);
        STR_selectedAnswer = string.Join(",", Answer);
    ...
    public void THI_TrackGameData(string analysis)
    {
       ...
       // Debug.Log("lws1 ==" + lws1);
        ...
        Debug.Log("lws1 toJson ==" + toJson); Debug.Log("STRL_gameData Before = " + STRL_gameData.Count);
        STRL_gameData.Add(toJson);
        Debug.Log("STRL_gameData After = " + STRL_gameData.Count);
