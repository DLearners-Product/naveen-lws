...
public class Audio_Fill : MonoBehaviour
{
   ...
   // public string[] STRA_Que;
    ...
    public void MoveNext()
    {
        if (I_questionCount < STRL_questions.Count - 1)
        {
            ...
            I_questionCount++;
            Debug.Log("OnGame = " + I_questionCount);
            THI_ShowQuestion();
        }
        else
        {
    ...
    public void THI_ShowQuestion()
    {
       ...
       // G_Question.transform.GetChild(0).transform.GetChild(0).GetComponent<AudioSource>().clip = ACA__questionClips[I_questionCount];
        if(ACA__questionClips[I_questionCount]!=null)
        {
            G_Question.transform.GetChild(0).transform.GetChild(1).GetComponent<AudioSource>().clip = ACA__questionClips[I_questionCount];
            G_Question.transform.GetChild(0).transform.GetChild(0).gameObject.SetActive(true);
        }
        else
        {
            G_Question.transform.GetChild(0).transform.GetChild(0).gameObject.SetActive(false);
        }
       

        TEX_question.text = STRL_questions[I_questionCount];
        ...
        {
            I_Dummy++;
            G_Options.transform.GetChild(i).name = STRL_options[I_Dummy];
            G_Options.transform.GetChild(i).transform.GetChild(0).GetComponent<Text>().text = STRL_options[I_Dummy];
            G_Options.transform.GetChild(i).GetComponent<AudioSource>().clip = ACA_optionClips[I_Dummy];
            G_Options.transform.GetChild(i).GetComponent<Image>().color = Color.white;
        }
        G_Selected = null;
        ...
        if (I_questionCount == 0)
        {
         ...
         // Debug.LogWarning("Playing Instruction audio = " + TEX_worksheetInstruction.gameObject.GetComponent<AudioSource>().clip.length);
    ...
    void BUT_Speaker()
    {
        G_Question.transform.GetChild(0).transform.GetChild(1).GetComponent<AudioSource>().Play();
    }
    ...
    public void BUT_Option()
    {
        ...
        string STR_dummy = STRL_questions[I_questionCount];

        if (STR_dummy.Contains("_"))
        {
