...
public class Match_Text_Image : MonoBehaviour
{
    ...
    void THI_ShowQuestion()
    {
        ...
        for (int i = 0; i < STRL_questions.Count; i++)
        {
            ...
            // GA_Questions[i].transform.GetChild(0).GetComponent<Image>().preserveAspect = true;
            GA_Questions[i].GetComponent<AudioSource>().clip = ACA__questionClips[i];
            GA_Questions[i].GetComponent<Image>().color = CLR_Colors[i];
        }

        if (SPRA_Options != null)
            for (int i = 0; i < SPRA_Options.Length; i++)
            {
                GA_Options[i].name = SPRA_Options[i].name;
                GA_Options[i].transform.GetChild(0).GetComponent<Image>().sprite = SPRA_Options[i];
                GA_Options[i].transform.GetChild(0).GetComponent<Image>().preserveAspect = true;
    ...
    public void BUT_option()
    {
        ...
        if (I_Dummy % 2 != 0)
        {
            ...
            // G_Selected1.GetComponent<Button>().interactable = false;
            ...
            //  STR_currentQuestionID = STRL_questionID[I_SelectedQcount];
            //  STR_currentQuestionAnswer = STRL_answers[I_SelectedQcount];
    ...
    public IEnumerator IN_downloadlImg()
    {
        ...
        //  THI_ShowQuestion();
        Invoke("THI_ShowQuestion",1f);
    }
    public void THI_getPreviewData()
    {
        MyJSON json = new MyJSON();
        json.Temp_type_1_LWS(MainController.instance.STR_previewJsonAPI, IL_numbers, STRL_instruction, STRL_instructionAudios, STRL_questions, STRL_answers, STRL_questionAudios, STRL_questionID, STRL_options, STRL_optionsAudios, STRL_details, STRL_cover_img_link,STRL_passageDetail);
