...
public class Match_Drag_Main : MonoBehaviour
{
    public static Match_Drag_Main OBJ_Match_Drag_Main;
    ...
    public void BUT_Restart()
    {
        ...
        {
            GA_Options[i].transform.SetParent(G_optParent.transform, false);
            GA_Options[i].transform.position = V2_OptionPos[i];
            GA_Options[i].transform.localScale = new Vector2(1.2f, 1.2f);
            // GA_Questions[i].transform.GetChild(1).GetComponent<Collider2D>().enabled = true;
            // GA_Options[i].GetComponent<Match_drag>().enabled = transform;
        }
    }
    ...
    {
        V2_OptionPos = new Vector2[STRL_options.Count];
        for (int i = 0; i < STRL_options.Count; i++)
        {
            V2_OptionPos[i] = GA_Options[i].transform.position;
        ...
        for (int i = 0; i < STRL_questions.Count; i++)
        {
           // GA_Questions[i].name = STRL_questions[i];
            GA_Questions[i].transform.GetChild(0).GetComponent<AudioSource>().clip = ACA__questionClips[i];
        }
    }
    ...
    public void THI_worksheetComplete()
    {
        ...
        for(int i=0;i<GA_Questions.Length;i++)
        {
            ...
            if (GA_Questions[i].transform.GetChild(1).transform.childCount != 0)
            {
               
                STR_selectedAnswer = GA_Questions[i].transform.GetChild(1).transform.GetChild(0).name;
    ...
    public IEnumerator IN_downloadlImg()
    {
        ...
        for (int i = 0; i < STRL_questions.Count; i++)
        {
            ...
            else
            {
                ...
                {
                    Debug.Log("Q_Image Asign");
                    GA_Questions[i].transform.GetChild(0).transform.GetChild(0).GetComponent<Image>().sprite = Sprite.Create(downloadedTexture, new Rect(0.0f, 0.0f, downloadedTexture.width, downloadedTexture.height), new Vector2(0.5f, 0.5f), 100.0f);
                    string[] Names = (STRL_questions[i].Split('/'));
                    string[] Finalname = (Names[Names.Length - 1].Split('.'));

                    GA_Questions[i].transform.GetChild(0).transform.GetChild(0).name = Finalname[0];
