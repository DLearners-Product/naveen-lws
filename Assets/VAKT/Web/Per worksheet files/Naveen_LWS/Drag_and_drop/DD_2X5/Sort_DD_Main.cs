...
using TMPro;
public class Sort_DD_Main : MonoBehaviour
{
    public static Sort_DD_Main OBJ_sort_DD_Main;
    ...
    void Start()
    {
        ...
        TEX_questionCount.text = "" + I_Qcount + "/" + (STRL_questions.Count);
        THI_gameData();
    }
    ...
    public void BUT_Restart()
    {
        ...
        {
            Destroy(GA_Questions[i]);
        }
        GA_Questions = new List<GameObject>();
        THI_ShowQuestion();
    }
    public void THI_BacktoOriginalpos()
    {
        ...
        for (int i = 0; i < STRL_questions.Count; i++)
        {
            ...
            {
                GA_TableCount[I_Tablecount].transform.GetChild(1).transform.GetChild(0).transform.GetChild(0).gameObject.GetComponent<VerticalLayoutGroup>().enabled = false;
                // Debug.Log("Debug.Log(already available other);");
                GA_Questions[i].transform.SetParent(GA_TableCount[I_Tablecount].transform.GetChild(1).transform.GetChild(0).transform.GetChild(0).transform, false);
                GA_Questions[i].transform.localScale = new Vector2(1f, 1f);
                
            }
        }
        GA_TableCount[I_Tablecount].transform.GetChild(1).transform.GetChild(0).transform.GetChild(0).gameObject.GetComponent<VerticalLayoutGroup>().enabled = true;
    }
    ...
    void THI_ShowQuestion()
    {
        ...
        for (int i=0;i<GA_TableCount.Length;i++)
        {
            GA_TableCount[i].SetActive(false);
        }
        GA_TableCount[I_Tablecount].SetActive(true);
       // Debug.Log("Table On = " + I_Tablecount);

      //  Debug.Log(STRL_options.Count + "  asdfasdf "+ STRL_questions.Count);

        GA_Options = new GameObject[STRL_options.Count];
        

        for (int i=0;i< GA_Options.Length;i++)
        {
            GA_Options[i] = (GA_TableCount[I_Tablecount].transform.GetChild(0).transform.GetChild(0).transform.GetChild(i).transform.GetChild(0).gameObject);
           
            for (int g = 1; g < GA_TableCount[I_Tablecount].transform.GetChild(0).transform.GetChild(0).transform.GetChild(i).childCount; g++)
            {
               // Debug.Log("OFF asset");
                GA_TableCount[I_Tablecount].transform.GetChild(0).transform.GetChild(0).transform.GetChild(i).transform.GetChild(g).gameObject.SetActive(false);
            }

            for (int k=1;k<I_Answercount+1; k++)
            {
              //  Debug.Log("On asset");
                GA_TableCount[I_Tablecount].transform.GetChild(0).transform.GetChild(0).transform.GetChild(i).transform.GetChild(k).gameObject.SetActive(true);
                GA_TableCount[I_Tablecount].transform.GetChild(0).transform.GetChild(0).transform.GetChild(i).transform.GetChild(k).GetComponent<Collider2D>().enabled = true;
            }
        }
       ...
       // int x= I_Answercount*STRL_options.Count;
        for (int i = 0; i < STRL_questions.Count; i++)
        {
            GameObject G_Dummy = Instantiate(G_Clone);
            G_Dummy.transform.SetParent(GA_TableCount[I_Tablecount].transform.GetChild(1).transform.GetChild(0).transform.GetChild(0).transform, false);
            G_Dummy.name = STRL_questions[i];
            G_Dummy.transform.GetChild(0).GetComponent<Text>().text = STRL_questions[i];
            G_Dummy.GetComponent<AudioSource>().clip = ACA__questionClips[i];
            GA_Questions.Add(G_Dummy);
        }
    }
    ...
    IEnumerator EN_GetData()
    {
        ...
        else
        {
           ...
           // StartCoroutine(IN_downloadlImg());
            ...
            {
                I_Tablecount = 0;
                I_Answercount = STRL_questions.Count / 2;
            }else
            if (STRL_options.Count == 3)
            {
                I_Tablecount = 1;
                I_Answercount = STRL_questions.Count / 3;
            }else
            if (STRL_options.Count == 4)
            {
                I_Tablecount = 2;
                I_Answercount = STRL_questions.Count / 4;
            }
            Debug.Log("Table = " + I_Tablecount);
           // THI_ShowQuestion();
        }
    }
